defmodule JambooWeb.PostHTML do
  ...
  @doc """
  Renders the voting block for a post.
  """
  attr :post, Jamboo.Content.Post, required: true

  def render_post_vote(assigns) do
    ~H"""
    <div class="flex flex-col items-center space-y-1 vote-container">
      ...
      >
        <.icon name="hero-arrow-up" class="size-6 text-gray-600 group-hover:text-orange-500" />
      </button>
      ...
      >
        <.icon name="hero-arrow-down" class="size-6 text-gray-600 group-hover:text-blue-500" />
      </button>
    </div>
    """
  end
